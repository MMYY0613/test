...
CONFIG = {
    ...
    "output_dir": "./output_mw_new_tmp_20260205",
    "test_steps": 4,
    "pc_max": 3,
    "p_lag": 1,
    "ridge": 1.0,
...
def plot_target_coefficients_all_in_one(
    coef_csv_path,
    output_dir,
    target_var="GDP",
    model_types=("マクロのみ", "PC1_DIFF"),   # ← ここ重要（後述）
    include_const=True,
    include_pc=True,                         # PCモデルだけPC列を足す
):
    apply_style_and_jpfont("default")
    ...
    df["EndDateStr"] = df["Window"].str.split("_").str[-1]
    df["Date"] = pd.to_datetime(df["EndDateStr"], format="%Y%m%d", errors="coerce")

    tdf = df[df["Target_Variable"] == target_var].copy()
    if tdf.empty:
        print(f"⚠️ Target_Variable={target_var} が見つかりません")
        return

    # マクロラグは全部
    lag_cols = sorted([c for c in tdf.columns if c.startswith("LAG_")])

    for mt in model_types:
        sub = tdf[tdf["Model_Type"] == mt].copy()
        if sub.empty:
            print(f"⚠️ Model_Type={mt} の {target_var} 係数が見つかりません（スキップ）")
            continue
        sub = sub.sort_values("Date")

        coef_cols = []
        if include_const and "CONST" in sub.columns:
            coef_cols.append("CONST")

        coef_cols += lag_cols

        # PC列はPCモデルのときだけ
        if include_pc and ("PC" in mt):
            pc_cols = sorted([c for c in sub.columns if c.startswith("PC")])
            coef_cols += pc_cols

        coef_cols = [c for c in coef_cols if c in sub.columns]
        ...
        for c in coef_cols:
            if sub[c].isna().all():
                continue
            ax.plot(sub["Date"], sub[c], marker="o", lw=1.6, ms=3, alpha=0.9, label=c)
        ...
        ax.set_title(f"{target_var}方程式 係数一覧（{mt}）", fontsize=13)
        ax.set_xlabel("Window終了日")
        ax.set_ylabel("Coefficient（標準化空間）")
        ax.grid(True, alpha=0.25)
        ax.legend(loc="upper left", bbox_to_anchor=(1.02, 1.00), frameon=True)

        plt.tight_layout(rect=[0, 0, 0.78, 1])
        save_path = output_dir / f"係数一覧_{target_var}_{mt}.png"
        plt.savefig(save_path, dpi=200, facecolor="white")
        plt.close()
        print(f"✅ 保存: {save_path}")

def coef_stability_rankings(
    coef_csv_path,
    output_dir,
    model_types=("マクロのみ", "PC1_DIFF"),
    targets=None,                 # Noneなら全Target
    include_cols_prefix=("CONST", "LAG_", "PC"),  # 係数列として見るprefix
    eps=1e-12
):
    """
    all_model_coefficients.csv から
    - std(β)
    - mean(|Δβ|)
    - sign change回数（0跨ぎ）
    を計算し、(Model_Type, Target_Variable)ごとにランキングCSVを吐く。
    """
    coef_csv_path = Path(coef_csv_path)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    if targets is not None:
        df = df[df["Target_Variable"].isin(targets)].copy()

    # 対象モデル
    if model_types is not None:
        df = df[df["Model_Type"].isin(model_types)].copy()

    if df.empty:
        print("⚠️ stability計算対象データが空です（model_types/targets を確認）")
        return

    # 係数列抽出（CONST, LAG_..., PC...）
    coef_cols = []
    for c in df.columns:
        if any([c.startswith(p) for p in include_cols_prefix]):
            coef_cols.append(c)

    if not coef_cols:
        print("⚠️ 係数列が見つかりません（CONST/LAG_/PC 列が無い）")
        return

    out_all = []

    # (Model, Target) ごとに評価
    keys = ["Model_Type", "Target_Variable"]
    for (mt, tv), g in df.groupby(keys):
        ...
        for col in coef_cols:
            ...
            # 2) mean(|Δ|)
            ds = s.diff()
            mean_abs_diff = float(ds.abs().mean(skipna=True))

            # 3) sign changes（0を跨ぐ回数）
            # sign = -1,0,+1 を作って、0は直前の符号に寄せる（ノイズで0が混じる対策）
            sign = np.sign(s.values)
            # 0を埋める：前方埋め→後方埋め
            sign = pd.Series(sign).replace(0, np.nan).ffill().bfill().fillna(0).values

            # 連続で符号が変わった回数
            sign_change = int(np.sum(sign[1:] * sign[:-1] < 0))

            # ついでに「0付近滞在率」も出すと便利（任意）
            # |β| < small_threshold の比率
            small_thr = 0.05
            near_zero_ratio = float((s.abs() < small_thr).mean())
            ...
            out_all.append({
                ...
                "N": int(s.notna().sum()),
                "std(beta)": std_beta,
                "mean_abs_diff(beta)": mean_abs_diff,
                "sign_change_count": sign_change,
                "near_zero_ratio(|beta|<0.05)": near_zero_ratio,
            })

    out_df = pd.DataFrame(out_all)
    if out_df.empty:
        print("⚠️ 出力が空です（係数がNaNだらけ/列名不一致の可能性）")
        return

    # 同一スケールで“暴れ”を並べたいので、スコア（単純合算）も作る
    # ※ sign_change を強めに効かせたいなら重みを変える
    out_df["instability_score"] = (
        out_df["std(beta)"] +
        out_df["mean_abs_diff(beta)"] +
        0.2 * out_df["sign_change_count"]
    )

    # 全体保存
    out_path_all = output_dir / "coef_stability_all.csv"
    out_df.sort_values(["Model_Type", "Target_Variable", "instability_score"], ascending=[True, True, False]) \
          .to_csv(out_path_all, index=False, encoding="utf-8-sig")
    print(f"✅ 保存: {out_path_all}")

    # (Model, Target)ごとにランキングCSVも吐く（見やすい）
    for (mt, tv), gg in out_df.groupby(["Model_Type", "Target_Variable"]):
        gg2 = gg.sort_values("instability_score", ascending=False)
        out_path = output_dir / f"coef_stability_rank_{tv}_{mt}.csv"
        gg2.to_csv(out_path, index=False, encoding="utf-8-sig")
        print(f"✅ 保存: {out_path}")

    # ついでに「上位だけ」まとめ
    topN = 10
    top_df = out_df.sort_values("instability_score", ascending=False).head(topN)
    top_path = output_dir / f"coef_stability_TOP{topN}.csv"
...
def main():
    ...
    if all_coefficients_records:
        ...
        targets = ["GDP", "NIKKEI", "USD_JPY", "UNEMP_RATE", "CPI", "JGB_1Y", "JGB_3Y", "JGB_10Y"]

        for t in targets:
            plot_target_coefficients_all_in_one(
                coef_csv_path=coef_path,
                output_dir=agg_root / "coefficients",
                target_var=t,
                model_types=("マクロのみ", "PC1_DIFF"),  # ← ここ重要（後述）
            )
        ...
        # 係数安定性ランキング（std / mean|Δ| / sign-change）
        coef_stability_rankings(
            coef_csv_path=agg_root / "coefficients" / "all_model_coefficients.csv",
            output_dir=agg_root / "coefficients" / "stability",
            model_types=("マクロのみ", "PC1_DIFF"),  # ←あなたの Model_Type 表記に合わせる
            targets=None  # 全ターゲット
        )
