...
CONFIG = {
    ...
    "output_dir": "./output_mw_new_tmp_6",
    "test_steps": 4,
    "pc_max": 3,
    "p_lag": 1,
    "ridge": 1.0,
...
CONFIG.update({
    ...
    "regime_q": 0.80,          # trainの外生スコア上位20%をレジーム1
    "regime_score": "absmax",  # absmax or l2
    "regime_hysteresis": None, # 例: (0.85, 0.75) など。まずはNoneでOK
})
...
# 2. ロジック関数
# =========================================================
def compute_regime_score(exog_df, mode="absmax"):
    """exog_df: (T, k)"""
    if exog_df is None or exog_df.empty:
        return pd.Series(np.nan, index=exog_df.index if exog_df is not None else None)
    if mode == "l2":
        return np.sqrt((exog_df**2).sum(axis=1))
    # default: absmax
    return exog_df.abs().max(axis=1)

def make_regime_indicator(score_all, score_train, q=0.8, hysteresis=None):
    # ★追加：q=1.0 以上は「RS無効」＝ I=0固定
    if q >= 1.0:
        I = pd.Series(0, index=score_all.index, dtype=int)
        return I, float("inf")  # thr は +inf 扱い

    # ★追加：q=0.0 以下は「常にレジーム」＝ I=1固定（必要なら）
    if q <= 0.0:
        I = pd.Series(1, index=score_all.index, dtype=int)
        return I, float("-inf")

    thr = score_train.quantile(q)

    if hysteresis is None:
        I = (score_all >= thr).astype(int)
        return I, float(thr)

    # 既存 hysteresis ロジックはそのまま
    q_high, q_low = hysteresis
    thr_hi = score_train.quantile(q_high)
    thr_lo = score_train.quantile(q_low)

    I = pd.Series(0, index=score_all.index, dtype=int)
    on = False
    for t in score_all.index:
        s = score_all.loc[t]
        if np.isnan(s):
            I.loc[t] = 0
            continue
        if (not on) and (s >= thr_hi):
            ...
            on = False
        I.loc[t] = 1 if on else 0
    return I, (float(thr_hi), float(thr_lo))

def augment_exog_with_regime(exog_df, I, suffix="_RS"):
    """exog_df に (I*exog) を追加して返す"""
    if exog_df is None or exog_df.empty:
        return exog_df, []
    I = I.reindex(exog_df.index).fillna(0).astype(float)
    exog_int = exog_df.mul(I, axis=0)
    exog_int.columns = [c + suffix for c in exog_df.columns]
    exog_full = pd.concat([exog_df, exog_int], axis=1)
    return exog_full, list(exog_int.columns)

def shade_regime(ax, I, color="lightgray", alpha=0.18):
    """背景をレジーム1で薄く塗る（連続区間ごとに axvspan）"""
    ...
    for dt, v in I.items():
        if (not on) and v == 1:
            ...
            ax.axvspan(start, dt, color=color, alpha=alpha, lw=0)
            on = False
    if on:
        ax.axvspan(start, I.index[-1], color=color, alpha=alpha, lw=0)

def build_exog_row_with_regime(te_s, d_next, combo, thr, mode="absmax"):
    base = te_s.loc[[d_next], combo]
    sc = compute_regime_score(base, mode=mode).iloc[0]
    I = 1.0 if (not np.isnan(sc) and sc >= thr) else 0.0
    rs = base * I
    rs.columns = [c + "_RS" for c in base.columns]
    full = pd.concat([base, rs], axis=1)
    return full.values
...
def make_test_design_with_context(m_cols, combo, ctx_s, te_s, means, stds, p, te_dates,
                                  use_rs=False, thr=None, score_mode="absmax"):
    """
    ctx_s, te_s: trainで求めたmeans/stdsで標準化済み（columnsは m_cols+combo を含む）
    use_rs=Trueなら exog=[exog, I*exog] を作って design 返す
    """
    ...
    if (not combo) or (not use_rs):
        Y, X, idx = make_design(te_raw2[m_cols], te_raw2[combo] if combo else None, p)
    else:
        exog_all = te_raw2[combo]
        score_all = compute_regime_score(exog_all, mode=score_mode)
        I_all = (score_all >= float(thr)).astype(int)
        exog_rs_all, _ = augment_exog_with_regime(exog_all, I_all, suffix="_RS")
        Y, X, idx = make_design(te_raw2[m_cols], exog_rs_all, p)

    is_test = idx.isin(te_dates)
    return Y[is_test], X[is_test], idx[is_test]

def save_and_plot_regime(score_full: pd.Series, I_full: pd.Series, thr: float,
                         save_dir: Path, title: str):
    save_dir.mkdir(parents=True, exist_ok=True)

    out_csv = save_dir / "regime_indicator.csv"
    pd.DataFrame({
        "Score": score_full,
        "Regime": I_full.astype(int),
        "Threshold": thr,
    }).to_csv(out_csv, encoding="utf-8-sig")

    # 図：上段score、下段レジーム(0/1)
    fig, axes = plt.subplots(2, 1, figsize=(14, 5), sharex=True)

    axes[0].plot(score_full.index, score_full.values, lw=1.6, color="black")
    axes[0].axhline(thr, ls="--", lw=1.2, color="red", alpha=0.8, label=f"thr={thr:.3f}")
    axes[0].set_ylabel("Regime score")
    axes[0].set_title(title)
    axes[0].grid(alpha=0.25)
    axes[0].legend(loc="upper left")

    # 0/1を帯で
    axes[1].plot(I_full.index, I_full.values, drawstyle="steps-post", lw=1.8, color="black")
    axes[1].set_ylim(-0.1, 1.1)
    axes[1].set_yticks([0, 1])
    axes[1].set_ylabel("Regime")
    axes[1].grid(alpha=0.25)

    # レジーム1を薄塗り（既存の shade_regime 使う）
    shade_regime(axes[0], I_full)
    shade_regime(axes[1], I_full)

    axes[1].xaxis.set_major_locator(mdates.YearLocator(2))
    axes[1].xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
    fig.tight_layout()
    fig.savefig(save_dir / "regime_summary.png", dpi=200)
    plt.close(fig)

def plot_regime_overall(output_dir: str, agg_root: Path, model_keyword: str = "PC1_DIFF"):
    """
    各 Window の各 combo の regime_indicator.csv を拾って結合し、
    model_keyword を含む combo だけを対象に全期間のレジーム帯を描く。
    """
    out_dir = Path(output_dir)
    files = list(out_dir.glob("Window_Test_*/*/regime/regime_indicator.csv"))
    if not files:
        print("⚠️ regime_indicator.csv が見つかりません")
        return

    pieces = []
    for f in files:
        # 親フォルダ名（combo名）でフィルタ
        combo_dir = f.parents[2].name  # Window_Test_xxx / <combo_dir> / regime / csv
        if model_keyword and (model_keyword not in combo_dir):
            continue

        df = pd.read_csv(f, index_col=0, parse_dates=True)
        df["ComboDir"] = combo_dir
        df["WindowDir"] = f.parents[3].name
        pieces.append(df[["Score", "Regime", "Threshold", "ComboDir", "WindowDir"]])

    if not pieces:
        print("⚠️ フィルタ後に対象がありません")
        return

    all_df = pd.concat(pieces).sort_index()

    # 同じ日付が複数ファイルに存在するので「最後優先」でユニーク化
    all_df = all_df[~all_df.index.duplicated(keep="last")]

    # まとめ図（Regimeのみ）
    fig, ax = plt.subplots(figsize=(14, 3))
    I = all_df["Regime"].astype(int)
    ax.plot(I.index, I.values, drawstyle="steps-post", color="black", lw=1.8)
    ax.set_ylim(-0.1, 1.1)
    ax.set_yticks([0, 1])
    ax.set_title(f"全期間レジーム推移（filter: {model_keyword}）")
    ...
    save_path = agg_root / "regime_overall.png"
    fig.savefig(save_path, dpi=200)
    plt.close(fig)

    # ついでにCSVも保存
    all_df.to_csv(agg_root / "regime_overall.csv", encoding="utf-8-sig")
    print(f"✅ 全期間レジームまとめを保存: {save_path}")
...
def main():
    ...
    for i in range(n_total - test_len + 1):
        ...
        for combo in combinations:
            ...
            if CONFIG.get("use_regime_switch", True) and combo:
                # trainスコア→thr
                score_tr = compute_regime_score(tr_s[combo], mode=CONFIG["regime_score"])
                I_tr, thr_info = make_regime_indicator(score_all=score_tr, score_train=score_tr,
                                                    q=CONFIG["regime_q"],
                                                    hysteresis=CONFIG.get("regime_hysteresis"))

                # hysteresis無し（thrがfloat）だけ、全期間を確定して保存
                if (thr_info is not None) and (not isinstance(thr_info, tuple)) and np.isfinite(float(thr_info)):
                    thr = float(thr_info)

                    # ★「そのWindowの全期間」でscore/Iを作る（標準化済みのexogで）
                    # 注意：tr_s/te_s は壁NaNを含む可能性があるので、ここは素直に全期間で作るのが安全
                    exog_full_std = ((pc_diff_all.loc[m_df_win.index, combo] - means[combo]) / stds[combo]).copy()
                    score_full = compute_regime_score(exog_full_std, mode=CONFIG["regime_score"])
                    I_full = (score_full >= thr).astype(int)

                    reg_dir = sub_dir / "regime"   # sub_dir は combo別フォルダ
                    save_and_plot_regime(
                        score_full=score_full,
                        I_full=I_full,
                        thr=thr,
                        save_dir=reg_dir,
                        title=f"{root_dir.name} | {m_name_display} | regime_q={CONFIG['regime_q']}"
                    )

            gdp_col = next(k for k in m_cols if meta[k]["orig"] == "GDP")
            gdp_j = m_cols.index(gdp_col)

            # === Regime indicator (trainでthr決めて、train+testに適用) ===
            I_tr = I_te = None
            thr_info = None

            if CONFIG.get("use_regime_switch", True) and combo:
                # スコアは「標準化後のexog」で作るとウィンドウ間で安定
                score_tr = compute_regime_score(tr_s[combo], mode=CONFIG["regime_score"])
                score_te = compute_regime_score(te_s[combo], mode=CONFIG["regime_score"])

                # 閾値は train スコアから
                I_tr, thr_info = make_regime_indicator(
                    score_all=score_tr, score_train=score_tr,
                    q=CONFIG["regime_q"],
                    hysteresis=CONFIG.get("regime_hysteresis")
                )

                # test も同じ閾値で判定（hysteresis None前提なら簡単）
                # hysteresis入れる場合は thr_info を使って同じロジックにしたいので、まずはNone推奨
                if isinstance(thr_info, tuple):
                    # hysteresis 使うなら、上の make_regime_indicator を score_all=concat で一回回すのが簡単
                    pass
                else:
                    thr = thr_info
                    I_te = (score_te >= thr).astype(int)

                # train側が少なすぎる場合はレジーム無効化（保険）
                if I_tr.sum() < 8:  # ここは適当に。少なすぎるとΔB推定が不安定
                    I_tr = None
                    I_te = None
                    thr_info = None
            ...
            # → Beta を推定（あなたの既存ロジック）# ===== RS一本：train design =====
            if combo:
                score_tr = compute_regime_score(tr_s[combo], mode=CONFIG["regime_score"])
                I_tr, thr_info = make_regime_indicator(
                    score_all=score_tr, score_train=score_tr,
                ...
                exog_tr_full, rs_cols = augment_exog_with_regime(tr_s[combo], I_tr, suffix="_RS")
            else:
                exog_tr_full, rs_cols = None, []

            param_names = (
                ["CONST"] + [f"LAG_{m}" for m in m_cols] +
                (list(combo) + rs_cols if combo else [])
            )

            # ===== RSモデル（外生だけレジームで変える：I*exog追加） =====
            if CONFIG.get("use_regime_switch", True) and combo and (I_tr is not None):
                exog_rs_tr, rs_cols = augment_exog_with_regime(tr_s[combo], I_tr, suffix="_RS")
                Y_tr_rs, X_tr_rs, idx_tr_rs = make_design(tr_s[m_cols], exog_rs_tr, CONFIG["p_lag"])

                # 係数名（保存・可視化用）
                param_names_rs = ["CONST"] + [f"LAG_{m}" for m in m_cols] + list(combo) + rs_cols

                # 推定（制約はまず“RS列は無制約”でOK。安定したら付ける）
                beta_list_rs = []
                ridge_aug_rs = np.sqrt(CONFIG["ridge"]) * np.eye(X_tr_rs.shape[1])
                X_aug_rs = np.vstack([X_tr_rs, ridge_aug_rs])

                for j, target_m_col in enumerate(m_cols):
                    orig_target = meta[target_m_col]["orig"]
                    Y_aug_rs = np.concatenate([Y_tr_rs[:, j], np.zeros(X_tr_rs.shape[1])])
                    lb = np.full(X_aug_rs.shape[1], -np.inf)
                    ub = np.full(X_aug_rs.shape[1],  np.inf)

                    # --- RS（I*exog）側： “絶対値を増やす方向” に制限（符号固定できるときだけ） ---
                    start_exog_idx = 1 + m_dim
                    start_rs_idx   = start_exog_idx + len(combo)   # RS列は base exog の直後

                    for k, exog_pc_name in enumerate(combo):
                        orig_exog = exog_pc_name.split("_")[0]     # "PC1" など
                        rs_idx = start_rs_idx + k                  # δ (= *_RS) の係数位置

                        # base側と同じルールで、符号制約（STRICT_CONSTRAINTS）を拾う
                        ...
                        # βが「正に固定」されている（下限>0）→ δ>=0（レジーム時に強くする）
                        if np.isfinite(lb_b) and (lb_b > 0):
                            lb[rs_idx] = 0.0

                        # βが「負に固定」されている（上限<0）→ δ<=0（レジーム時により負にする）
                        if np.isfinite(ub_b) and (ub_b < 0):
                            ub[rs_idx] = 0.0

                    # ★RS（I*exog）側は一旦ノー制約（必要なら後で追加）
                    # 例：レジーム時は“より強くなる”だけ許すなら lb>=0 を入れる等
                    # ラグ制約（既存 LAG_CONSTRAINTS 適用）
                    for pred_i, pred_m_col in enumerate(m_cols):
                        pred_orig = meta[pred_m_col]["orig"]
                        target_idx = 1 + pred_i
                        bound = (LAG_CONSTRAINTS.get((orig_target, pred_orig)) or
                                ...
                                LAG_CONSTRAINTS.get(("*", "*")))
                        if bound:
                            lb[target_idx], ub[target_idx] = bound

                    res_rs = lsq_linear(X_aug_rs, Y_aug_rs, bounds=(lb, ub), lsmr_tol='auto')
                    if not res_rs.success:
                        res_rs = lsq_linear(X_aug_rs, Y_aug_rs, bounds=(-np.inf, np.inf), lsmr_tol='auto')
                    beta_list_rs.append(res_rs.x)

                    # 係数CSVに残す（既存 all_coefficients_records と同じ方式）
                    record_rs = {
                        "Window": root_dir.name,
                        "Model_Type": m_name_display + " + RS",
                        "Target_Variable": orig_target,
                    }
                    for name, val in zip(param_names_rs, res_rs.x):
                        record_rs[name] = val
                    all_coefficients_records.append(record_rs)

                Beta_rs = np.array(beta_list_rs).T
                # ---- RS有効化フラグ確定 ----
                rs_enabled = (
                    (Beta_rs is not None) and
                    (thr_info is not None) and
                    (not isinstance(thr_info, tuple)) and
                    np.isfinite(float(thr_info))
                )
                thr_val = float(thr_info) if rs_enabled else None
            ...
            Y_te_rs = X_te_rs = idx_te_rs = None
            te_rmse_rs = np.nan

            if pos - p >= 0:
                ctx_dates = common_idx[pos-p:pos]
                ...
                # まずフラグを作る（後述）
                # base（ベースは必ず False）
                Y_te, X_te, idx_te = make_test_design_with_context(
                    m_cols, combo, ctx_s, te_s, means, stds, p, te_dates,
                    use_rs=False
                )

                # RS（Beta_rs がある場合だけ）
                if (Beta_rs is not None) and combo and (thr_info is not None) and (not isinstance(thr_info, tuple)):
                    use_rs_flag = (CONFIG.get("use_regime_switch", True) and combo and (thr_info is not None) and (not isinstance(thr_info, tuple)))
                    thr_val = float(thr_info) if use_rs_flag else None
                    Y_te_rs, X_te_rs, idx_te_rs = make_test_design_with_context(
                        m_cols, combo, ctx_s, te_s, means, stds, p, te_dates,
                        use_rs=use_rs_flag, thr=thr_val, score_mode=CONFIG["regime_score"]
                    )
                    if len(Y_te_rs) > 0:
                        te_resid_rs = Y_te_rs - X_te_rs @ Beta_rs
                        te_rmse_rs = float(np.sqrt(np.nanmean(te_resid_rs**2)))
            else:
                ...
                if Beta_rs is not None:
                    # ctx+test の exog（標準化済み）を作る
                    exog_ctx = ctx_s[combo] if combo else pd.DataFrame(index=ctx_s.index)
                    exog_te  = te_s[combo]  if combo else pd.DataFrame(index=te_s.index)
                    exog_all = pd.concat([exog_ctx, exog_te], axis=0)

                    # trainで決めたthrで判定（thr_infoがfloat前提）
                    thr = float(thr_info)
                    score_all = compute_regime_score(exog_all, mode=CONFIG["regime_score"])
                    I_all = (score_all >= thr).astype(int)

                    exog_rs_all, rs_cols = augment_exog_with_regime(exog_all, I_all, suffix="_RS")

                    # make_design
                    Y_te_rs, X_te_rs, idx_te_rs = make_design(te_raw2[m_cols], exog_rs_all, p)

                    is_test = idx_te_rs.isin(te_dates)
                    Y_te_rs = Y_te_rs[is_test]
                    X_te_rs = X_te_rs[is_test]
                    idx_te_rs = idx_te_rs[is_test]
            ...
            if Beta_rs is not None:
                tr_resid_rs = Y_tr_rs - X_tr_rs @ Beta_rs
                tr_rmse_rs = float(np.sqrt(np.nanmean(tr_resid_rs**2)))

                # AIC(rs)
                sigma_rs = (tr_resid_rs.T @ tr_resid_rs) / len(Y_tr_rs)
                sign_rs, logdet_rs = np.linalg.slogdet(sigma_rs)
                if sign_rs > 0:
                    aic_rs = len(Y_tr_rs) * logdet_rs + 2 * Beta_rs.size
                else:
                    eigs = np.linalg.eigvalsh(sigma_rs)
                    valid = eigs[eigs > 1e-15]
                    aic_rs = (len(Y_tr_rs) * np.sum(np.log(valid)) + 2 * Beta_rs.size) if len(valid) else np.nan

                # 最大固有値(rs)
                A1_rs = Beta_rs[1:1+m_dim, :].T
                max_eig_rs = float(np.max(np.abs(np.linalg.eigvals(A1_rs))))

                summary_list.append({
                    "モデル構成": m_name_display + " + RS",
                    "訓練RMSE": round(tr_rmse_rs, 4),
                    "訓練RMSE_GDP": np.nan,
                    "予測RMSE": round(te_rmse_rs, 4) if not np.isnan(te_rmse_rs) else np.nan,
                    "予測RMSE_GDP": np.nan,
                    "RMSE比": round(te_rmse_rs / tr_rmse_rs, 2) if tr_rmse_rs > 0 and not np.isnan(te_rmse_rs) else np.nan,
                    "AIC": round(aic_rs, 2) if not np.isnan(aic_rs) else np.nan,
                    "最大固有値": round(max_eig_rs, 3),
                })

            # --- 予測結果の可視化 (逐次予測 & 全体表示版) ---
            # ループ前に「この予測で使う係数」と「外生行の作り方」を決める
            beta_for_pred = Beta
            use_rs_in_pred = False

            if rs_enabled and (Beta_rs is not None):
                beta_for_pred = Beta_rs
                use_rs_in_pred = True
            ...
            # RSが有効なら、RS用の設計行列を使う
            X_for_pred  = X_te_rs if use_rs_in_pred else X_te
            idx_for_pred = idx_te_rs if use_rs_in_pred else idx_te

            if X_for_pred is not None and len(X_for_pred) > 0:
                curr_X = X_for_pred[0:1, :]

                for t in range(len(idx_for_pred)):
                    pred_t = curr_X @ beta_for_pred
                    y_te_pred_list.append(pred_t)

                    if t < len(idx_for_pred) - 1:
                        next_lag_y = pred_t

                        if combo:
                            d_next = idx_for_pred[t+1]
                            if use_rs_in_pred:
                                next_exog = build_exog_row_with_regime(
                                    te_s, d_next, combo,
                                    thr=thr_val,
                                    mode=CONFIG["regime_score"]
                                )  # (1, 2k)
                            else:
                                next_exog = te_s.loc[[d_next], combo].values  # (1, k)
                        else:
            ...
            if y_te_pred_scaled is not None and i != 0:
                ...
                for i_m, m_col in enumerate(m_cols):
                    ...
                    # 背景塗り（test期間だけでもOK）
                    if Beta_rs is not None:
                        # te_actual_dates 上の I を作って塗る
                        base_exog_te = te_s.loc[te_actual_dates, combo]
                        score_te = compute_regime_score(base_exog_te, mode=CONFIG["regime_score"])
                        I_plot = (score_te >= float(thr_info)).astype(int)
                        shade_regime(ax, I_plot.reindex(full_display_range).fillna(0).astype(int))
            ...
            if CONFIG["do_irf"] and combo:
                k = len(combo)
                B_base = Beta[(1+m_dim):(1+m_dim+k), :].T  # base exogだけ
                # B_rs   = Beta[(1+m_dim+k):(1+m_dim+2*k), :].T  # regime exog側（使いたければ）
                for j, pc_label in enumerate(combo):
                    pc_folder = sub_dir / f"Shock_{pc_label}"; pc_folder.mkdir(parents=True, exist_ok=True)
                    impact = np.zeros((13, m_dim)); impact[1] = B_base[:, j]
    ...
    plot_regime_overall(CONFIG["output_dir"], agg_root, model_keyword="PC1_DIFF")
    # もしベース(マクロのみ)を見たいなら:
    # plot_regime_overall(CONFIG["output_dir"], agg_root, model_keyword="BASE_VAR_ONLY")
