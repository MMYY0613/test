...
# =========================================================
# ★ここが肝：sector/macro を同じ index に揃えてから split する
# =========================================================
def prepare_aligned(df_raw: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DatetimeIndex]:
    """
    返り値:
      - sector_df: SECTOR_COLS の数値化 + dropna + sort
      - macro_df : MACRO_COLS  の数値化 + dropna + sort
      - common_idx: sector/macro 両方が揃う index（ここに完全に揃える）
    """
    df_raw = df_raw.replace([np.inf, -np.inf], np.nan)
    df_raw = ensure_macro_columns(df_raw)

    # 必須列チェック
    missing_sector = [c for c in SECTOR_COLS if c not in df_raw.columns]
    missing_macro  = [c for c in MACRO_COLS  if c not in df_raw.columns]
    if missing_sector:
        raise ValueError(f"SECTOR_COLS に必要な列が無い: {missing_sector}")
    if missing_macro:
        raise ValueError(f"MACRO_COLS に必要な列が無い: {missing_macro}")

    sector_df = safe_numeric(df_raw[SECTOR_COLS].copy(), SECTOR_COLS).dropna().sort_index()
    macro_df  = safe_numeric(df_raw[MACRO_COLS ].copy(), MACRO_COLS ).dropna().sort_index()

    common_idx = sector_df.index.intersection(macro_df.index)
    sector_df = sector_df.loc[common_idx]
    macro_df  = macro_df.loc[common_idx]

    if len(common_idx) < 10:
        raise ValueError(f"共通indexが短すぎる: {len(common_idx)}。欠損(dropna)が多すぎないか確認して。")

    return sector_df, macro_df, common_idx

def idx_pos(index: pd.DatetimeIndex, date_str: str, side: str = "left") -> int:
    d = pd.to_datetime(date_str)
    return int(np.searchsorted(index.values, np.datetime64(d), side=side))
...
) -> tuple[int,int,int,pd.Timestamp,pd.Timestamp,pd.Timestamp,pd.Timestamp]:
    """
    split は “common_idx 上の位置” で管理する（これでPCAもdiagnosticsも一致する）
    train: [ts:te_excl)
    test : [te_excl: te_excl+test_size)
    """
    ts = idx_pos(common_idx, train_start, side="left")
    if test_start is None:
        te_incl = idx_pos(common_idx, train_end, side="right") - 1  # train_end を含める
        te_excl = te_incl + 1
    else:
        te_incl = idx_pos(common_idx, train_end, side="right") - 1
        te_excl = idx_pos(common_idx, test_start, side="left")  # test_start 明示ならそこから
        if te_excl <= ts:
            raise ValueError("test_start が train_start より前/同じになってる。")
        # train_end は “含む” が、test_start が優先なので train_end より後に来てもOK（ただし直感に反するなら注意）

    if te_excl <= ts:
        raise ValueError("train期間が空。train_start/train_end を見直して。")

    ttest = int(test_size)
    if te_excl + ttest > len(common_idx):
        raise ValueError("test がデータ末尾を超える。test_size か期間を見直して。")

    train_start_pos = ts
    train_end_pos   = te_excl
    test_start_pos  = te_excl

    tr_s = common_idx[train_start_pos]
    tr_e = common_idx[train_end_pos - 1]
    te_s = common_idx[test_start_pos]
    te_e = common_idx[test_start_pos + ttest - 1]
    return train_start_pos, train_end_pos, ttest, tr_s, tr_e, te_s, te_e
...
# =========================================================
# PCA保存（gridとdiagで形式統一：1 split 1 folder）
# =========================================================
def save_pca_outputs(
    ...
    meta: dict,
    *,
    jp_label: dict[str, str] | None = None,
    style: str = "diag",  # "diag" or "grid"
):
    out_dir.mkdir(parents=True, exist_ok=True)

    pd.Series(meta).to_csv(out_dir / "meta_pca.csv", encoding="utf-8-sig")
    pc_all.to_csv(out_dir / ("PC_scores.csv" if style == "diag" else "セクター_主成分スコア.csv"), encoding="utf-8-sig")
    loading.to_csv(out_dir / ("PC_loadings.csv" if style == "diag" else "セクター_負荷量.csv"), encoding="utf-8-sig")
    pca_info.to_csv(out_dir / ("PC_variance.csv" if style == "diag" else "主成分_寄与率.csv"), encoding="utf-8-sig")

    title = f"PCA score time series (train fit → test transform, test={meta['test_size']})"
    plot_pc_timeseries(pc_all, split_ts, title, out_dir / "PC_scores_timeseries.png")

    # loadings top10
    for pc_name in loading.columns:
        if style == "grid":
            plot_loading_top10(loading, pc_name, out_dir / f"負荷量_{sanitize_name(pc_name)}_上位10.png", jp_label=jp_label or {})
        else:
            top = loading[pc_name].abs().sort_values(ascending=False).head(10).index
            plot_df = loading.loc[top, [pc_name]].copy()
            ...
            ax.axhline(0.0, linewidth=1.2, linestyle=":")
            ax.grid(True, axis="y", linestyle="--", linewidth=0.6, alpha=0.5)
            plt.title(f"{pc_name} loadings (top10)")
            plt.tight_layout()
            plt.savefig(out_dir / f"loadings_{sanitize_name(pc_name)}_top10.png", dpi=200)
            plt.close()

    # 係数(a,b)も残す（grid側でやってたやつを共通化）
    ...
    b_ser = pd.Series(b_vec, name="const_b")

    a_mat.to_csv(out_dir / "PC_linear_a.csv", encoding="utf-8-sig")
    b_ser.to_csv(out_dir / "PC_linear_b.csv", encoding="utf-8-sig")
...
def fit_varx_ridge(df_train_std: pd.DataFrame,
    ...
    for lag in range(1, p + 1):
        ...
        A_list[lag] = coef.loc[rows].values.T

    if exog_cols:
        rows_B = [f"B_{c}" for c in exog_cols]
        B_mat = coef.loc[rows_B].values.T
    else:
...
def forecast_varx(model: dict,
    ...
    for h in range(steps):
        ...
        if exog:
            y_hat += B @ x_now
        preds.append(y_hat)
        y_hist = pd.concat([y_hist, pd.DataFrame([y_hat], index=[df_future_exog_std.index[h]], columns=endog)], axis=0)
...
def irf_exog_no_ci(A_list: dict[int, np.ndarray], B: np.ndarray, p: int, steps: int) -> np.ndarray:
    m, kx = B.shape
...
def save_irf_outputs(
    ...
    out_dir: Path,
    steps: int,
    plot_only_gdp: bool = False,
    gdp_level_init: float = 500000.0,
):
    H = int(steps)
    h = np.arange(H + 1)
    ...
    for j, xname in enumerate(exog_cols):
        ...
        targets = (["GDP_LOGDIFF"] if "GDP_LOGDIFF" in endog_cols else []) if plot_only_gdp else endog_cols
        ...
        for ycol in targets:
            ...
            plt.title(f"IRF: {xname} shock (+1σ raw={sx:.4g}) → {ycol} (σ={sy:.4g}) [RAW]")
            plt.xlabel("horizon")
        ...
        # --- GDP level (Δlogの累積) は shockごとに1回だけ ---
        if "GDP_LOGDIFF" in endog_cols:
            g = irf_df["GDP_LOGDIFF"].values
            gdp_level = float(gdp_level_init) * np.exp(np.cumsum(g))

            pd.DataFrame({"GDP_LEVEL": gdp_level}, index=h).to_csv(
                sub / "irf_GDP_LEVEL_from_init500000.csv",
                encoding="utf-8-sig"
            )
...
# =========================================================
# exog 組み合わせ（timeout回避のためモード選べる）
# =========================================================
def pc_exog_sets(pc_cols: list[str], mode: str = "full") -> list[list[str]]:
    """
    mode:
      - "full"   : PC1..PCk を全部入れた1本だけ（最速）
      - "single" : [PC1], [PC2], ... のみ
      - "pair"   : 2個組だけ
      - "all"    : 全非空部分集合（遅い/timeoutしやすい）
    """
    mode = str(mode).lower()
    if mode == "full":
        return [pc_cols[:]]
    ...
    if mode == "pair":
        if len(pc_cols) < 2:
            return [pc_cols[:]]
        return [list(x) for x in itertools.combinations(pc_cols, 2)]
    if mode == "all":
        ...
        return combos
    raise ValueError("mode must be one of: full/single/pair/all")


# =========================================================
# 1 split で PCA + diagnostics をまとめて出す（splitは common_idx のposで統一）
# =========================================================
def run_one_split(
    ...
    *,
    ...
    # ★重要：split位置は「prepare_alignedで作った共通idx（= macro_df.index）」基準のまま使う
    idx_full = macro_df.index  # = sector_df.index と一致している前提（prepare_alignedが保証）

    tr_idx = idx_full[train_start_pos:train_end_pos]
    te_idx = idx_full[train_end_pos:train_end_pos + int(test_size)]

    df_train_macro_raw = macro_df.loc[tr_idx].copy()
    df_test_macro_raw  = macro_df.loc[te_idx].copy()

    # pc_all は train+test 範囲の index だけ持ってるので、ここはlocでOK
    df_train_pc_raw = pc_all.loc[tr_idx].copy()
    df_test_pc_raw  = pc_all.loc[te_idx].copy()

    # デバッグ用（残してOK）
    print("[DEBUG] len(idx_full)=", len(idx_full),
        " len(pc_all)=", len(pc_all),
        " train=", len(tr_idx),
        " test=", len(te_idx))
    ...
    exog_sets = pc_exog_sets(pc_cols, mode=exog_mode)
    ...
    for exog_cols in exog_sets:
        ...
        ratio = float(rmse_test_mean / rmse_train_mean) if rmse_train_mean > 0 else np.nan
        ...
        # save rmse table
        rmse_tbl = pd.DataFrame({"rmse_train": rmse_train_each, "rmse_test": rmse_test_each})
        ...
        if do_plots:
            ...
            df_all_macro = pd.concat([df_train_macro_raw, df_test_macro_raw], axis=0)
        ...
        meta = pd.Series({
            ...
            "rmse_ratio_test_over_train": ratio,
    ...
    print("DONE split ->", out_dir.resolve())


# =========================================================
# 入口：日付でsplit指定（ここが一番事故らない）
# =========================================================
def run_one_split_by_dates(
    df_raw: pd.DataFrame,
    *,
    train_start: str,
    train_end: str,
    ...
    sector_df, macro_df, common_idx = prepare_aligned(df_raw)

    train_start_pos, train_end_pos, ttest, tr_s, tr_e, te_s, te_e = build_split(
    ...
    print("SPLIT (common_idx-based)")
    print(f"  train: {tr_s:%Y-%m-%d} .. {tr_e:%Y-%m-%d}  (pos {train_start_pos}:{train_end_pos})")
    print(f"  test : {te_s:%Y-%m-%d} .. {te_e:%Y-%m-%d}  (size {ttest})")
...
def parse_args():
    ...
    # one split (dates)
    s1 = sub.add_parser("one_split", help="日付で train/test を指定して PCA+diagnostics を1回出す")
    s1.add_argument("--data", default=DATA_PATH)
    s1.add_argument("--train-start", required=True)
    s1.add_argument("--train-end", required=True)
    s1.add_argument("--test-size", type=int, default=4)
    s1.add_argument("--test-start", default=None)
    s1.add_argument("--cum-th", type=float, default=0.90)
    s1.add_argument("--pc-use", type=int, default=3)
    s1.add_argument("--p-lag", type=int, default=2)
    s1.add_argument("--ridge", type=float, default=1.0)
    s1.add_argument("--exog-mode", default="full", choices=["full", "single", "pair", "all"])
    s1.add_argument("--irf-steps", type=int, default=12)
    s1.add_argument("--no-irf", action="store_true")
    s1.add_argument("--no-plots", action="store_true")
...
def main():
    ...
    if args.cmd == "one_split":
        run_one_split_by_dates(
            df_raw=df_raw,
