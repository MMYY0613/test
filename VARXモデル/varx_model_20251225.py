...
def plot_train_fit(
    ...
    """
    # make_design の都合で、fitted は先頭数期が欠けているので index を揃える
...
# Beta 行列（k × m）
Beta = model["coef"].values  # coef は (k, m)

# 標準化スケールでの当てはめ値
Yhat_train_std = Xv_train @ Beta  # shape: (T_eff, m)
...
for col in RET_COLS:
    y_true = train_raw.loc[idx_train, col]  # 元スケールの真の値
